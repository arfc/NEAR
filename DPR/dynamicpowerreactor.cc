...
DynamicPowerReactor::DynamicPowerReactor(cyclus::Context* ctx)
    : cyclus::Facility(ctx),
      n_assem_batch(0),
      assem_size(0),
      n_assem_core(0),
...
#pragma cyclus def clone cycamore::DynamicPowerReactor

#pragma cyclus def schema cycamore::DynamicPowerReactor

#pragma cyclus def annotations cycamore::DynamicPowerReactor

#pragma cyclus def infiletodb cycamore::DynamicPowerReactor

#pragma cyclus def snapshot cycamore::DynamicPowerReactor

#pragma cyclus def snapshotinv cycamore::DynamicPowerReactor

#pragma cyclus def initinv cycamore::DynamicPowerReactor

void DynamicPowerReactor::InitFrom(DynamicPowerReactor* m) {
  #pragma cyclus impl initfromcopy cycamore::DynamicPowerReactor
  cyclus::toolkit::CommodityProducer::Copy(m);
}

void DynamicPowerReactor::InitFrom(cyclus::QueryableBackend* b) {
  #pragma cyclus impl initfromdb cycamore::DynamicPowerReactor
...
void DynamicPowerReactor::EnterNotify() {
...
bool DynamicPowerReactor::CheckDecommissionCondition() {
  return core.count() == 0 && spent.count() == 0;
}

void DynamicPowerReactor::Tick() {
...
std::set<cyclus::RequestPortfolio<Material>::Ptr> DynamicPowerReactor::GetMatlRequests() {
...
void DynamicPowerReactor::GetMatlTrades(
...
void DynamicPowerReactor::AcceptMatlTrades(const std::vector<
...
std::set<cyclus::BidPortfolio<Material>::Ptr> DynamicPowerReactor::GetMatlBids(
...
void DynamicPowerReactor::Tock() {
  ...
  double adjusted_power = 1.0 * power_cap;

  if (reactor_life < power_percent_list.size()){
    adjusted_power = power_percent_list[reactor_life] * adjusted_power;
  }

  if (cycle_step >= 0 && cycle_step < cycle_time &&
      core.count() == n_assem_core) {
    cyclus::toolkit::RecordTimeSeries<cyclus::toolkit::POWER>(this, adjusted_power);
    cyclus::toolkit::RecordTimeSeries<double>("supplyPOWER", this, adjusted_power);
...
void DynamicPowerReactor::Transmute() { Transmute(n_assem_batch); }

void DynamicPowerReactor::Transmute(int n_assem) {
...
std::map<std::string, MatVec> DynamicPowerReactor::PeekSpent() {
...
bool DynamicPowerReactor::Discharge() {
...
std::string DynamicPowerReactor::fuel_incommod(Material::Ptr m) {
  int i = res_indexes[m->obj_id()];
  if (i >= fuel_incommods.size()) {
    throw KeyError("cycamore::DynamicPowerReactor - no incommod for material object");
  }
  return fuel_incommods[i];
}

std::string DynamicPowerReactor::fuel_outcommod(Material::Ptr m) {
  int i = res_indexes[m->obj_id()];
  if (i >= fuel_outcommods.size()) {
    throw KeyError("cycamore::DynamicPowerReactor - no outcommod for material object");
  }
  return fuel_outcommods[i];
}

std::string DynamicPowerReactor::fuel_inrecipe(Material::Ptr m) {
  int i = res_indexes[m->obj_id()];
  if (i >= fuel_inrecipes.size()) {
    throw KeyError("cycamore::DynamicPowerReactor - no inrecipe for material object");
  }
  return fuel_inrecipes[i];
}

std::string DynamicPowerReactor::fuel_outrecipe(Material::Ptr m) {
  int i = res_indexes[m->obj_id()];
  if (i >= fuel_outrecipes.size()) {
    throw KeyError("cycamore::DynamicPowerReactor - no outrecipe for material object");
  }
  return fuel_outrecipes[i];
}

double DynamicPowerReactor::fuel_pref(Material::Ptr m) {
  int i = res_indexes[m->obj_id()];
...
void DynamicPowerReactor::index_res(cyclus::Resource::Ptr m, std::string incommod) {
  ...
  throw ValueError(
      "cycamore::DynamicPowerReactor - received unsupported incommod material");
}

std::map<std::string, MatVec> DynamicPowerReactor::PopSpent() {
  MatVec mats = spent.PopN(spent.count());
  std::map<std::string, MatVec> mapped;
  for (int i = 0; i < mats.size(); i++) {
    std::string commod = fuel_outcommod(mats[i]);
...
void DynamicPowerReactor::PushSpent(std::map<std::string, MatVec> leftover) {
  std::map<std::string, MatVec>::iterator it;
...
void DynamicPowerReactor::RecordSideProduct(bool produce){
  if (hybrid_){
    double value;
    for (int i = 0; i < side_products.size(); i++) {
      if (produce){
      ...
      context()
          ->NewDatum("DynamicPowerReactorSideProducts")
          ->AddVal("AgentId", id())
...
void DynamicPowerReactor::Record(std::string name, std::string val) {
  context()
      ->NewDatum("DynamicPowerReactorEvents")
      ->AddVal("AgentId", id())
      ->AddVal("Time", context()->time())
      ->AddVal("Event", name)
      ->AddVal("Value", val)
...
void DynamicPowerReactor::RecordPosition() {
...
extern "C" cyclus::Agent* ConstructDynamicPowerReactor(cyclus::Context* ctx) {
  return new DynamicPowerReactor(ctx);
}
