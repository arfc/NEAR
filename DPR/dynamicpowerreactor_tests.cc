...
namespace cycamore {
namespace dynamicpowerreactortests {
...
// delay.
TEST(DynamicPowerReactorTests, JustInTimeOrdering) {
  std::string config =
  ...
  int simdur = 50;
  cyclus::MockSim sim(cyclus::AgentSpec(":cycamore:DynamicPowerReactor"), config, simdur);
...
// cycle.
TEST(DynamicPowerReactorTests, BatchSizes) {
  std::string config =
...
// cycle is honored.
TEST(DynamicPowerReactorTests, RefuelTimes) {
  std::string config =
     "  <fuel_inrecipes>  <val>uox</val>      </fuel_inrecipes>  "
     "  <fuel_outrecipes> <val>spentuox</val> </fuel_outrecipes>  "
     "  <fuel_incommods>  <val>uox</val>      </fuel_incommods>  "
...
// tests that a DynamicPowerReactor decommissions on time without producing
// power at the end of its lifetime.
TEST(DynamicPowerReactorTests, DecomTimes) {
  std::string config =
     "  <fuel_inrecipes>  <val>uox</val>      </fuel_inrecipes>  "
     "  <fuel_outrecipes> <val>spentuox</val> </fuel_outrecipes>  "
     "  <fuel_incommods>  <val>uox</val>      </fuel_incommods>  "
  ...
  int simdur = 12;
  int lifetime = 7;
  cyclus::MockSim sim(cyclus::AgentSpec(":cycamore:DynamicPowerReactor"), config, simdur, lifetime);
  sim.AddSource("uox").Finalize();
  sim.AddRecipe("uox", c_uox());
  sim.AddRecipe("spentuox", c_spentuox());
  int id = sim.Run();
...
// Tests if a DynamicPowerReactor produces power at the time of its decommission
// given a refuel_time of zero.
TEST(DynamicPowerReactorTests, DecomZeroRefuel) {
  std::string config =
     "  <fuel_inrecipes>  <val>uox</val>      </fuel_inrecipes>  "
     "  <fuel_outrecipes> <val>spentuox</val> </fuel_outrecipes>  "
     "  <fuel_incommods>  <val>uox</val>      </fuel_incommods>  "
...
TEST(DynamicPowerReactorTests, OrderAtRefuelStart) {
  std::string config =
     "  <fuel_inrecipes>  <val>uox</val>      </fuel_inrecipes>  "
     "  <fuel_outrecipes> <val>spentuox</val> </fuel_outrecipes>  "
     "  <fuel_incommods>  <val>uox</val>      </fuel_incommods>  "
...
// tests that the DynamicPowerReactor handles requesting multiple types of fuel correctly
// - with proper inventory constraint honoring, etc.
TEST(DynamicPowerReactorTests, MultiFuelMix) {
  std::string config =
  ...
  // the DynamicPowerReactor can take on a single time step - to test that inventory
  // capacity constraints are being set properly.  It is also important that
  // each source is smaller capacity thatn the DynamicPowerReactor orders on each time
...
// tests that the DynamicPowerReactor halts operation when it has no more room in its
// spent fuel inventory buffer.
TEST(DynamicPowerReactorTests, FullSpentInventory) {
  std::string config =
     "  <fuel_inrecipes>  <val>uox</val>      </fuel_inrecipes>  "
     "  <fuel_outrecipes> <val>spentuox</val> </fuel_outrecipes>  "
     "  <fuel_incommods>  <val>uox</val>      </fuel_incommods>  "
...
// tests that the DynamicPowerReactor shuts down, ie., does not generate power, when the
// spent fuel inventory is full and the core cannot be unloaded.
TEST(DynamicPowerReactorTests, FullSpentInventoryShutdown) {
  std::string config =
    " <fuel_inrecipes> <val>uox</val> </fuel_inrecipes> "
    " <fuel_outrecipes> <val>spentuox</val> </fuel_outrecipes> "
    " <fuel_incommods> <val>uox</val> </fuel_incommods> "
...
// tests that the DynamicPowerReactor cycle is delayed as expected when it is unable to
...
// start time.
TEST(DynamicPowerReactorTests, FuelShortage) {
  std::string config =
     "  <fuel_inrecipes>  <val>uox</val>      </fuel_inrecipes>  "
     "  <fuel_outrecipes> <val>spentuox</val> </fuel_outrecipes>  "
     "  <fuel_incommods>  <val>uox</val>      </fuel_incommods>  "
...
TEST(DynamicPowerReactorTests, DischargedFuelTransmute) {
  std::string config =
     "  <fuel_inrecipes>  <val>uox</val>      </fuel_inrecipes>  "
     "  <fuel_outrecipes> <val>spentuox</val> </fuel_outrecipes>  "
     "  <fuel_incommods>  <val>uox</val>      </fuel_incommods>  "
...
// simultaneously.
TEST(DynamicPowerReactorTests, SpentFuelProperCommodTracking) {
  std::string config =
     "  <fuel_inrecipes>  <val>uox</val>      <val>mox</val>      </fuel_inrecipes>  "
     "  <fuel_outrecipes> <val>spentuox</val> <val>spentmox</val> </fuel_outrecipes>  "
     "  <fuel_incommods>  <val>uox</val>      <val>mox</val>      </fuel_incommods>  "
...
// correctly and the DynamicPowerReactor could segfault.  Check that this doesn't happen.
TEST(DynamicPowerReactorTests, PrefChange) {
...
TEST(DynamicPowerReactorTests, RecipeChange) {
  // it is important that the fuel_prefs not be present in the config below.
  std::string config =
     "  <fuel_inrecipes>  <val>lwr_fresh</val>  </fuel_inrecipes>  "
     "  <fuel_outrecipes> <val>lwr_spent</val>  </fuel_outrecipes>  "
...
TEST(DynamicPowerReactorTests, Retire) {
  std::string config =
     "  <fuel_inrecipes>  <val>lwr_fresh</val>  </fuel_inrecipes>  "
     "  <fuel_outrecipes> <val>lwr_spent</val>  </fuel_outrecipes>  "
     "  <fuel_incommods>  <val>enriched_u</val> </fuel_incommods>  "
  ...
  int cycle_time = 7;
  int refuel_time = 0;
  cyclus::MockSim sim(cyclus::AgentSpec(":cycamore:DynamicPowerReactor"), config, dur, life);
  sim.AddSource("enriched_u").Finalize();
  sim.AddSink("waste").Finalize();
  sim.AddRecipe("lwr_fresh", c_uox());
  sim.AddRecipe("lwr_spent", c_spentuox());
  ...
  // DynamicPowerReactor should stop requesting new fresh fuel as it approaches retirement
  int nassem_recv =
  ...
  // DynamicPowerReactor should discharge all fuel before/by retirement
  conds.clear();
  conds.push_back(Cond("SenderId", "==", id));
  qr = sim.db().Query("Transactions", &conds);
  EXPECT_EQ(nassem_recv, qr.rows.size())
  ...
  // DynamicPowerReactor should record power entry on the time step it retires if operating
...
TEST(DynamicPowerReactorTests, PositionInitialize) {
  std::string config =
     "  <fuel_inrecipes>  <val>lwr_fresh</val>  </fuel_inrecipes>  "
     "  <fuel_outrecipes> <val>lwr_spent</val>  </fuel_outrecipes>  "
     "  <fuel_incommods>  <val>enriched_u</val> </fuel_incommods>  "
...
TEST(DynamicPowerReactorTests, PositionInitialize2) {
  std::string config =
     "  <fuel_inrecipes>  <val>lwr_fresh</val>  </fuel_inrecipes>  "
     "  <fuel_outrecipes> <val>lwr_spent</val>  </fuel_outrecipes>  "
     "  <fuel_incommods>  <val>enriched_u</val> </fuel_incommods>  "
...
TEST(DynamicPowerReactorTests, ByProduct) {
  std::string config =
     "  <fuel_inrecipes>  <val>uox</val>      </fuel_inrecipes>  "
     "  <fuel_outrecipes> <val>spentuox</val> </fuel_outrecipes>  "
     "  <fuel_incommods>  <val>uox</val>      </fuel_incommods>  "
  ...
  std::vector<Cond> conds;
  // test if it produces side products only when DynamicPowerReactor is running
  int quantity = 10;
  conds.push_back(Cond("Value", "==", quantity));
  QueryResult qr = sim.db().Query("DynamicPowerReactorSideProducts", &conds);
  EXPECT_EQ(5, qr.rows.size());

  // test if it doesn't produce side products when DynamicPowerReactor is refueling
  conds.clear();
  conds.push_back(Cond("Value", "==", 0));
  qr = sim.db().Query("DynamicPowerReactorSideProducts", &conds);
  EXPECT_EQ(5, qr.rows.size());
}

TEST(DynamicPowerReactorTests, MultipleByProduct) {
  std::string config =
     "  <fuel_inrecipes>  <val>uox</val>      </fuel_inrecipes>  "
     "  <fuel_outrecipes> <val>spentuox</val> </fuel_outrecipes>  "
     "  <fuel_incommods>  <val>uox</val>      </fuel_incommods>  "
  ...
  std::vector<Cond> conds;
  // test if it produces heat when DynamicPowerReactor is running
  int quantity = 10;
  ...
  // test if it produces water when DynamicPowerReactor is running
  conds.clear();
  quantity = 100;
