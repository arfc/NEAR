...
#pragma cyclus def clone cycamore::Ever

#pragma cyclus def schema cycamore::Ever

#pragma cyclus def annotations cycamore::Ever

#pragma cyclus def infiletodb cycamore::Ever

#pragma cyclus def snapshot cycamore::Ever

#pragma cyclus def snapshotinv cycamore::Ever

#pragma cyclus def initinv cycamore::Ever

void Ever::InitFrom(Ever* m) {
  #pragma cyclus impl initfromcopy cycamore::Ever
  cyclus::toolkit::CommodityProducer::Copy(m);
}

void Ever::InitFrom(cyclus::QueryableBackend* b) {
  #pragma cyclus impl initfromdb cycamore::Ever
...
void Ever::EnterNotify() {
  ...
  if (recipe_update_in.size() != nu) {
    ss << "prototype '" << prototype() << "' has " << recipe_update_in.size()
       << " recipe_update_in vals, expected " << nu << "\n";
  }
  if (recipe_update_out.size() != nu) {
...
bool Ever::CheckDecommissionCondition() {
...
std::set<cyclus::RequestPortfolio<Material>::Ptr> Ever::GetMatlRequests() {
...
void Ever::AcceptMatlTrades(const std::vector<
...
std::set<cyclus::BidPortfolio<Material>::Ptr> Ever::GetMatlBids(
...
void Ever::Transmute() { Transmute(n_assem_batch); }

void Ever::Transmute(int n_assem) {
...
std::map<std::string, MatVec> Ever::PeekSpent() {
...
std::string Ever::fuel_incommod(Material::Ptr m) {
  int i = res_indexes[m->obj_id()];
  if (i >= fuel_incommods.size()) {
    throw KeyError("cycamore::Ever - no incommod for material object");
  }
  return fuel_incommods[i];
}

std::string Ever::fuel_outcommod(Material::Ptr m) {
  int i = res_indexes[m->obj_id()];
  if (i >= fuel_outcommods.size()) {
    throw KeyError("cycamore::Ever - no outcommod for material object");
  }
  return fuel_outcommods[i];
}

std::string Ever::fuel_inrecipe(Material::Ptr m) {
  int i = res_indexes[m->obj_id()];
  if (i >= fuel_inrecipes.size()) {
    throw KeyError("cycamore::Ever - no inrecipe for material object");
  }
  return fuel_inrecipes[i];
}

std::string Ever::fuel_outrecipe(Material::Ptr m) {
  int i = res_indexes[m->obj_id()];
  if (i >= fuel_outrecipes.size()) {
    throw KeyError("cycamore::Ever - no outrecipe for material object");
  }
  return fuel_outrecipes[i];
}

double Ever::fuel_pref(Material::Ptr m) {
  int i = res_indexes[m->obj_id()];
...
void Ever::index_res(cyclus::Resource::Ptr m, std::string incommod) {
  ...
  throw ValueError(
      "cycamore::Ever - received unsupported incommod material");
}

std::map<std::string, MatVec> Ever::PopSpent() {
  MatVec mats = spent.PopN(spent.count());
  std::map<std::string, MatVec> mapped;
  for (int i = 0; i < mats.size(); i++) {
    std::string commod = fuel_outcommod(mats[i]);
...
void Ever::PushSpent(std::map<std::string, MatVec> leftover) {
  std::map<std::string, MatVec>::iterator it;
...
void Ever::RecordSideProduct(bool produce){
  if (hybrid_){
    double value;
    for (int i = 0; i < side_products.size(); i++) {
      if (produce){
...
void Ever::Record(std::string name, std::string val) {
  context()
      ->NewDatum("EverEvents")
      ->AddVal("AgentId", id())
      ->AddVal("Time", context()->time())
...
extern "C" cyclus::Agent* ConstructEver(cyclus::Context* ctx) {
  return new Ever(ctx);
}
