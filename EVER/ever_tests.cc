...
// delay.
TEST(EverTests, JustInTimeOrdering) {
  std::string config =
  ...
  int simdur = 50;
  cyclus::MockSim sim(cyclus::AgentSpec(":cycamore:Ever"), config, simdur);
...
TEST(EverTests, DecomTimes) {
  ...
  int simdur = 12;
  int lifetime = 7;
  cyclus::MockSim sim(cyclus::AgentSpec(":cycamore:Ever"), config, simdur, lifetime);
  sim.AddSource("uox").Finalize();
  sim.AddRecipe("uox", c_uox());
  sim.AddRecipe("spentuox", c_spentuox());
  int id = sim.Run();
...
TEST(EverTests, DecomZeroRefuel) {
  std::string config =
     "  <fuel_inrecipes>  <val>uox</val>      </fuel_inrecipes>  "
     "  <fuel_outrecipes> <val>spentuox</val> </fuel_outrecipes>  "
     "  <fuel_incommods>  <val>uox</val>      </fuel_incommods>  "
...
TEST(EverTests, OrderAtRefuelStart) {
  std::string config =
     "  <fuel_inrecipes>  <val>uox</val>      </fuel_inrecipes>  "
     "  <fuel_outrecipes> <val>spentuox</val> </fuel_outrecipes>  "
     "  <fuel_incommods>  <val>uox</val>      </fuel_incommods>  "
...
TEST(EverTests, FullSpentInventory) {
  std::string config =
     "  <fuel_inrecipes>  <val>uox</val>      </fuel_inrecipes>  "
     "  <fuel_outrecipes> <val>spentuox</val> </fuel_outrecipes>  "
     "  <fuel_incommods>  <val>uox</val>      </fuel_incommods>  "
...
TEST(EverTests, FullSpentInventoryShutdown) {
  std::string config =
    " <fuel_inrecipes> <val>uox</val> </fuel_inrecipes> "
    " <fuel_outrecipes> <val>spentuox</val> </fuel_outrecipes> "
    " <fuel_incommods> <val>uox</val> </fuel_incommods> "
...
TEST(EverTests, DischargedFuelTransmute) {
  std::string config =
     "  <fuel_inrecipes>  <val>uox</val>      </fuel_inrecipes>  "
     "  <fuel_outrecipes> <val>spentuox</val> </fuel_outrecipes>  "
     "  <fuel_incommods>  <val>uox</val>      </fuel_incommods>  "
...
// simultaneously.
TEST(EverTests, SpentFuelProperCommodTracking) {
  std::string config =
     "  <fuel_inrecipes>  <val>uox</val>      <val>mox</val>      </fuel_inrecipes>  "
     "  <fuel_outrecipes> <val>spentuox</val> <val>spentmox</val> </fuel_outrecipes>  "
     "  <fuel_incommods>  <val>uox</val>      <val>mox</val>      </fuel_incommods>  "
...
TEST(EverTests, RecipeUpdate) {
  ...
  // check that received recipe updated to water
  conds.clear();
  ...
  // check that sent recipe updated to water
  conds.clear();
...
TEST(EverTests, Retire) {
  ...
  int cycle_time = 7;
  int refuel_time = 0;
  cyclus::MockSim sim(cyclus::AgentSpec(":cycamore:Ever"), config, dur, life);
  sim.AddSource("enriched_u").Finalize();
  sim.AddSink("waste").Finalize();
  sim.AddRecipe("lwr_fresh", c_uox());
  sim.AddRecipe("lwr_spent", c_spentuox());
...
TEST(EverTests, PositionInitialize) {
  std::string config =
     "  <fuel_inrecipes>  <val>lwr_fresh</val>  </fuel_inrecipes>  "
     "  <fuel_outrecipes> <val>lwr_spent</val>  </fuel_outrecipes>  "
     "  <fuel_incommods>  <val>enriched_u</val> </fuel_incommods>  "
...
TEST(EverTests, PositionInitialize2) {
  std::string config =
     "  <fuel_inrecipes>  <val>lwr_fresh</val>  </fuel_inrecipes>  "
     "  <fuel_outrecipes> <val>lwr_spent</val>  </fuel_outrecipes>  "
     "  <fuel_incommods>  <val>enriched_u</val> </fuel_incommods>  "
...
TEST(EverTests, ByProduct) {
  ...
  QueryResult qr = sim.db().Query("EverSideProducts", &conds);
  EXPECT_EQ(5, qr.rows.size());

  // test if it doesn't produce side products when reactor is refueling
  conds.clear();
  conds.push_back(Cond("Value", "==", 0));
  qr = sim.db().Query("EverSideProducts", &conds);
  EXPECT_EQ(5, qr.rows.size());
}

TEST(EverTests, MultipleByProduct) {
  std::string config =
     "  <fuel_inrecipes>  <val>uox</val>      </fuel_inrecipes>  "
     "  <fuel_outrecipes> <val>spentuox</val> </fuel_outrecipes>  "
     "  <fuel_incommods>  <val>uox</val>      </fuel_incommods>  "
